...
(** [write_line serial message] writes [message] to the connection [serial], asynchronously, returning the promise of a new connection ([t Lwt.t]). If a write fails, an attempt is made to establish a new connection, which is then returned in place of the old connection. *)
